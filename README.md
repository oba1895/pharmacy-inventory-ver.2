...
- **閲覧モード**: ログイン不要（在庫・履歴の閲覧のみ）
- **管理者モード**: 管理者PINでログイン（入出庫入力・編集・削除・マスタ管理）

管理者PINは `backend/.env` の `ADMIN_PIN` で設定します（未設定時の初期値は `1234`）。

> ⚠️ **導入時は必ずPINを変更してください。** `.env` はGitに含まれません（各環境で個別に設定）。
